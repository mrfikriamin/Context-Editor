import type { ApiEndpoint, ConfigVariable, ContextState, DataEntity, GeneratedFile, SelectedTech, TypeDetails, ScriptTypeDetails, TerraformTypeDetails, ApiServiceTypeDetails, CliTypeDetails, AutomationTypeDetails, ProjectType, StackCategoryId } from "./types"
import { isUIProject } from "./types"
import type { DesktopTypeDetails } from "./types"
import { profileFromTypeDetails, desktopProfile } from "./desktop-frameworks"
import type { DesktopFrameworkProfile } from "./desktop-frameworks"
import { STACK_CATEGORIES, TECH_OPTIONS } from "./stack-options"
import { DOC_INGESTION_TOOLS, GRAPH_TOOLS, LINT_TOOLS, resolveContextTool } from "./context-tools"
import type { ContextToolOption } from "./context-tools"
...
function mdList(items: string[], fallback = "_None specified yet._"): string {
  if (items.length === 0) return fallback
  return items.map((i) => `- ${i}`).join("\n")
}

function val(v: string, fallback = "_TBD_"): string {
  return v.trim() ? v.trim() : fallback
}

// Profile prose like "Qt Widgets (dense desktop UIs)" nests badly inside
// parenthesized sentences — strip the parentheticals when embedding.
function plainText(t: string): string {
  return t.replace(/\s*\([^)]*\)/g, "").trim()
}

// User text placed inside a markdown table cell must not break the row.
function cell(v: string, fallback = "_—_"): string {
  const t = v.trim()
  if (!t) return fallback
  return t.replace(/\|/g, "\\|").replace(/\r?\n/g, "<br>")
}

function techLabel(selected: SelectedTech[]): string {
  if (selected.length === 0) return "_None selected_"
  return selected
    .map((t) => (t.config ? `${t.label} (${t.config})` : t.label))
    .join(", ")
}

function getSelectedForCategory(selected: SelectedTech[], categoryId: string): SelectedTech[] {
  const optionIds = new Set((TECH_OPTIONS[categoryId] ?? []).map((o) => o.id))
  return selected.filter((t) => optionIds.has(t.techId))
}

function mermaidBlock(code: string): string {
  const trimmed = code.trim()
  if (!trimmed) return "_No diagram provided yet. Add one in the Mermaid Editor step._"
  return `\`\`\`mermaid\n${trimmed}\n\`\`\``
}

function projectTypeLabel(kind: TypeDetails["kind"]): string {
...
function typeNounPhrase(kind: ProjectType): string {
  const phrases: Record<ProjectType, string> = {
...
function detailProp(td: TypeDetails, key: string): string {
  if (td.kind === "full-application") return ""
  const d = td.details as unknown as Record<string, string>
  return d[key] ?? ""
}
...
const RELEVANT_CATEGORIES: Record<ProjectType, StackCategoryId[]> = {
...
function intro(s: ContextState, text: string): string {
  return s.contextTools?.compactInstructions ? "" : `${text}\n\n`
}

function graphToolForType(kind: ProjectType, profile?: DesktopFrameworkProfile | null): string {
  switch (kind) {
...
function toolMeta(t: ContextToolOption): string {
  const parts = [t.url, t.command ? `\`${t.command}\`` : ""].filter(Boolean)
  return parts.length ? ` (${parts.join("; ")})` : ""
}

function graphToolText(s: ContextState): string {
  const ct = s.contextTools
  const profile = profileFromTypeDetails(s.idea.typeDetails)
  if (!ct) return graphToolForType(s.idea.typeDetails.kind, profile)
  const t = resolveContextTool(GRAPH_TOOLS, ct.codebaseGraphTool ?? "auto", ct.codebaseGraphCustom ?? "")
  if (t.id === "auto") return graphToolForType(s.idea.typeDetails.kind, profile)
...
function langProfile(kind: ProjectType, profile?: DesktopFrameworkProfile | null): LangProfile {
  switch (kind) {
    case "desktop-application":
      return {
        typing: [
...
export function dataModelProfile(kind: ProjectType): DataModelProfile {
  switch (kind) {
    case "desktop-application":
      return {
...
function parseRiskLine(line: string): RiskItem {
...
function riskItems(text: string): RiskItem[] {
  return listFromText(text).map(parseRiskLine).filter((r) => r.risk)
}

function hasDataModel(s: ContextState): boolean {
  return s.dataModel.entities.some(
    (entity) => entity.name.trim() || entity.description.trim() || entity.fields.some((field) => field.name.trim())
  )
}

function endpointRows(endpoints: ApiEndpoint[], fallback: string): string {
  const rows = endpoints.filter((endpoint) => endpoint.method.trim() || endpoint.path.trim())
  if (!rows.length) return fallback
  return rows
    .map(
      (endpoint) =>
        `| ${cell(endpoint.method, "_TBD_")} | ${cell(endpoint.path, "_TBD_")} | ${cell(endpoint.request, "_TBD_")} | ${cell(endpoint.response, "_TBD_")} | ${cell(endpoint.auth, "_TBD_")} |`
    )
    .join("\n")
}

function relevantStackOpenDecisions(s: ContextState): string[] {
  const relevant = new Set<string>(RELEVANT_CATEGORIES[s.idea.typeDetails.kind])
...
function hasProjectConfig(s: ContextState): boolean {
  ...
  if (td.kind === "terraform") {
    return Boolean(
      td.details.backendStorage.trim() ||
  ...
  if (td.kind === "desktop-application") {
    return Boolean(
      td.details.targetOS.trim() ||
        td.details.buildTool.trim() ||
        td.details.packaging.trim() ||
        td.details.codeSigning.trim() ||
        td.details.autoUpdate.trim() ||
        td.details.dataStorage.trim() ||
        td.details.distribution.trim()
    )
  }
  return Boolean(
    td.details.envVars.trim() ||
      td.details.configFile.trim() ||
      td.details.secretsHandling.trim() ||
      td.details.logging.trim() ||
...
function apiEndpoints(s: ContextState): ApiEndpoint[] {
  return s.idea.typeDetails.kind === "api-service" ? s.idea.typeDetails.details.endpoints : []
}

function openDecisionItems(s: ContextState): string[] {
...
function openDecisionsMarkdown(s: ContextState, fallback = "_No blocking open decisions detected._"): string {
...
function stackConfigRows(vars: ConfigVariable[]): string {
...
function configurationRows(s: ContextState): string {
  const td = s.idea.typeDetails
  switch (td.kind) {
...
function configurationSection(s: ContextState): string {
...
function securitySection(s: ContextState): string {
  const td = s.idea.typeDetails
  let specific: string[] = []
  switch (td.kind) {
    case "full-application":
    ...
    case "desktop-application":
      specific = [
        ...(profileFromTypeDetails(td)?.securityBullets ?? []),
...
function operationsSection(s: ContextState): string {
  const td = s.idea.typeDetails
  ...
  switch (td.kind) {
    case "desktop-application": {
      const p = desktopProfile(td.details.framework)
...
function typeDetailsSection(td: TypeDetails): string {
  switch (td.kind) {
    case "python-script":
    case "bash-script":
    case "powershell-script":
...
function desktopDetailsSection(d: DesktopTypeDetails): string {
  const p = desktopProfile(d.framework)
...
function scriptDetailsSection(d: ScriptTypeDetails, kind: ProjectType): string {
...
function terraformDetailsSection(d: TerraformTypeDetails): string {
...
function apiDetailsSection(d: ApiServiceTypeDetails): string {
...
function cliDetailsSection(d: CliTypeDetails): string {
...
function automationDetailsSection(d: AutomationTypeDetails): string {
...
function sanitizeErId(value: string, fallback: string): string {
  const sanitized = value.replace(/[^a-zA-Z0-9_]/g, "_").replace(/^_+|_+$/g, "")
  return sanitized || fallback
}

function relationshipDiagram(entities: DataEntity[], relationships: string[]): string {
...
function dataModel(s: ContextState): string {
  const profile = dataModelProfile(s.idea.typeDetails.kind)
  const entities = s.dataModel.entities.filter(
    (entity) => entity.name.trim() || entity.description.trim() || entity.fields.some((field) => field.name.trim())
  )
        ...
        .map((entity, index) => {
          ...
          const fieldRows = fields.length
            ? fields
                .map(
                  (field) =>
                    `| ${cell(field.name, "_TBD_")} | ${cell(field.type, "_TBD_")} | ${field.required ? "Yes" : "No"} | ${cell(field.notes, "_—_")} |`
                )
                .join("\n")
...
| _TBD_ | _TBD_ | _TBD_ | ${profile.emptyDirective} |`
  ...
  const relationshipSection =
    s.idea.typeDetails.kind === "terraform"
      ? ""
      : `## Relationships
${relationships.length ? mdList(relationships) : "_None specified yet._"}`

  const diagramSection =
    profile.showErDiagram && entities.length && relationships.length
      ? `## ER Diagram
...
function dataFlowForType(s: ContextState): string {
  const { stack, idea } = s
  ...
  switch (idea.typeDetails.kind) {
    case "desktop-application": {
      ...
      const p = desktopProfile(d.framework)
      return `## Data Flow
1. The user interacts with the UI (${plainText(val(d.uiToolkit, p.uiToolkit))}).
...
function folderStructureForType(kind: ProjectType, profile?: DesktopFrameworkProfile | null): string {
  switch (kind) {
    case "desktop-application":
      if (profile) return profile.folderStructure
...
function projectOverview(s: ContextState): string {
  ...
  const mvpRows = mvp.length
    ? mvp
        .map(
          (f) =>
            `| ${cell(f.name)} | ${f.description.trim() ? cell(f.description) : "_To be detailed during implementation._"} | Planned |`
        )
        .join("\n")
    : "| _None specified yet._ | _Add MVP features in the Features step._ | Planned |"

  const showUI = isUIProject(idea.projectType)
  // Native-UI desktop frameworks (Qt, C#) keep the design files except the
  // web component registry — the reading strategy must say so explicitly.
  const nativeDesktopUI = (() => {
    const dp = profileFromTypeDetails(idea.typeDetails)
    return !!dp && !dp.usesWebTech
  })()
...
7. ${
      showUI
        ? nativeDesktopUI
          ? "`ui-tokens.md`, `ui-rules.md` — before any interface work (`ui-registry.md` does not apply to native UI)"
          : "`ui-tokens.md`, `ui-rules.md`, `ui-registry.md` — before any interface work"
        : "`ui-rules.md` — before writing user-facing output, logs, or errors"
    }
...
function architecture(s: ContextState): string {
  const { stack, mermaid, idea } = s
  const kind = idea.typeDetails.kind
  const auth = getSelectedForCategory(stack.selected, "auth")
  const validation = getSelectedForCategory(stack.selected, "validation")
  const relevant = new Set<string>(RELEVANT_CATEGORIES[kind])

  const dprofile = profileFromTypeDetails(idea.typeDetails)
  ...
  const stackTable = STACK_CATEGORIES
    .filter((c) => c.id !== "other")
  ...
  if (kind === "full-application" || kind === "api-service") {
    implementationNotes.push(
      `Authentication approach: ${auth.length ? techLabel(auth) : "_TBD — decide before building protected features._"}`
    )
  }
  implementationNotes.push(
    `Validation approach: ${validation.length ? `validate all external input at the boundary with ${techLabel(validation)}.` : "validate all external input at the boundary; choose the validation tooling in Phase 0."}`,
...
System boundaries: everything in this repository is inside the boundary; ${
    kind === "terraform"
      ...
      : kind === "full-application"
        ...
        : kind === "desktop-application"
          ...
          : "external services, data sources, and the scheduler/invoker are outside"
  }. Confirm before adding any integration that crosses it.${graphBlock}
...
function codeStandards(s: ContextState): string {
  ...
  const runtimeFromDetails =
    idea.typeDetails.kind === "terraform"
      ? "Terraform (HCL)"
...
function notApplicable(title: string): string {
  return `# ${title}
...
function uiTokens(s: ContextState): string {
  if (!isUIProject(s.idea.projectType)) {
    return notApplicable("UI Tokens")
  }
  const { look } = s
  const dp = profileFromTypeDetails(s.idea.typeDetails)
  const nativeNote =
    dp && !dp.usesWebTech
      ? `> Native-UI note: these tokens express design intent, not CSS. Map them to ${dp.label}'s styling system (${
          dp.id === "qt" ? "QSS / QML properties" : "XAML resources and styles"
        }) and keep the mapping consistent.\n\n`
      : ""
  return `# UI Tokens

${intro(s, "Design tokens only — usage rules live in `ui-rules.md`.")}${nativeNote}## Brand Adjectives
...
function outputInterfaceRules(s: ContextState): string {
  const td = s.idea.typeDetails
  if (td.kind === "terraform") {
    return `# UI Rules
...
function uiRules(s: ContextState): string {
  if (!isUIProject(s.idea.projectType)) {
...
function uiRegistry(s: ContextState): string {
  if (!isUIProject(s.idea.projectType)) {
    return notApplicable("UI Registry")
  }
  const dprofile = profileFromTypeDetails(s.idea.typeDetails)
  if (dprofile && !dprofile.usesWebTech) {
    return `# UI Registry

> **Not applicable** — this desktop application uses ${dprofile.label} native UI (${plainText(dprofile.uiToolkit)}), not a web component registry.
...
function optionalContextTools(s: ContextState): string {
  const ct = s.contextTools
  if (!ct) return ""
  const tools: string[] = []
  if (ct.docIngestion) {
    const t = resolveContextTool(DOC_INGESTION_TOOLS, ct.docIngestionTool, ct.docIngestionCustom)
...
- Don't: render on every read — validation is only needed after a change.`)
  ...
  if (ct.markdownLinting) {
    const t = resolveContextTool(LINT_TOOLS, ct.lintTool, ct.lintCustom)
...
function libraryDocs(s: ContextState): string {
  const { stack } = s
  const cats = STACK_CATEGORIES.filter((c) => c.id !== "other" && (s.idea.typeDetails.kind === "full-application" || c.id !== "frontend"))

  const mainRows = cats
    .flatMap((cat) =>
      getSelectedForCategory(stack.selected, cat.id).map(
        (t) => `| ${cell(t.label)} | ${cat.label} | ${t.config ? cell(t.config) : cell(cat.description)} | Official docs required before use. |`
      )
    )
    .join("\n")
  ...
  const allRows =
    [mainRows, libRows].filter(Boolean).join("\n") ||
  ...
  const INSTALLABLE = new Set<StackCategoryId>(["frontend", "backend", "orm", "auth", "validation", "testing"])
  const installList = [
    ...cats
...
function buildPlan(s: ContextState): string {
  ...
  const setupOpenDecisions = openDecisions.filter(
    (decision) =>
      !decision.startsWith("Define or explicitly defer") &&
...
- [ ] Final pass on \`progress-tracker.md\`: statuses, decision log, change log`
  ...
  const contextPrep = [
    ...
    ct?.docIngestion
      ? `- [ ] Convert referenced external documents to markdown with ${resolveContextTool(DOC_INGESTION_TOOLS, ct.docIngestionTool, ct.docIngestionCustom).label} — only the sections needed`
      : "",
...
## MVP Features
${mdList(
  mvp.map((f) => (f.description.trim() ? `${f.name} — ${f.description.trim()}` : f.name)),
  "_None specified yet. Add features in the Features step._"
)}
...
function progressTracker(s: ContextState): string {
  ...
  const qualityRows = [
    ...
    ct?.markdownLinting
      ? `| Markdown lint clean (${resolveContextTool(LINT_TOOLS, ct.lintTool, ct.lintCustom).label}) | _Run after edits_ |`
      : "",
...
function mNode(id: string, text: string, shape: "rect" | "db" | "diamond" = "rect"): string {
...
export function starterDiagram(s: ContextState): string {
  const td = s.idea.typeDetails
  ...
  const chainFeatures = (fromId: string, toId: string) => {
    let prev = fromId
    feats.forEach((f, i) => {
...
export function packManifest(s: ContextState): string {
  const name = val(s.idea.projectName, "Untitled Project")
  const ct = s.contextTools
  const enabledTools = [
    ct?.docIngestion &&
      `${resolveContextTool(DOC_INGESTION_TOOLS, ct.docIngestionTool, ct.docIngestionCustom).label} (document → markdown ingestion)`,
    ct?.codebaseGraph && `codebase graph / dependency report (${graphToolText(s)})`,
    ct?.mermaidValidation && "Mermaid CLI diagram validation",
    ct?.markdownLinting &&
      `${resolveContextTool(LINT_TOOLS, ct.lintTool, ct.lintCustom).label} for context files`,
...
export function generateFiles(s: ContextState): GeneratedFile[] {
  return [
